...
component {
    ...
    function index() {
		var ret = {};
        ret.incomplete = ormExecuteQuery("from Applications where SpecialUseNumber is null order by Updated desc");
        ret.recent = ormExecuteQuery("from Applications order by Updated desc",false,{maxResults=5});
        ret.totalPendingRecords = ormExecuteQuery("select count(*) from Applications where Status.Code = 'P'")[1];
        ret.totalActiveRecords = ormExecuteQuery("select count(*) from Applications where Status.Code = 'A'")[1];
        ret.totalInactiveRecords = ormExecuteQuery("select count(*) from Applications where Status.Code = 'I'")[1];
        ret.totalDeniedRecords = ormExecuteQuery("select count(*) from Applications where Status.Code = 'D'")[1];
